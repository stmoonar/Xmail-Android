...
public class InboxActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url = apiUrl + "/mail/listReceived?"
                        + "username=" + username;
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if (result.getCode() == 1) {
                                Toast.makeText(InboxActivity.this, "获取收件箱成功", Toast.LENGTH_SHORT).show();
                                //显示邮件列表
                                showEmails(result.getData());
                            } else {
                                Toast.makeText(InboxActivity.this, "获取收件箱失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } catch (IOException e) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(InboxActivity.this, "网络连接失败", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
    ...
    protected void showEmails(Object data) {
        ...
        EmailAdapter adapter = new EmailAdapter(mailList, new EmailAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                ...
                Toast.makeText(InboxActivity.this, "正在打开该邮件", Toast.LENGTH_SHORT).show();
                    ...
                    public void run() {
                        try {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    //打开FullEmailActivity
                                    Intent intent = new Intent(InboxActivity.this, FullEmailActivity.class);
